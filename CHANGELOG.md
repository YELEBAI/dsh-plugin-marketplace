...
- 修复 DSH Store #716 揭示的包导出缺陷：构建并随包提供真实类型声明，纠正 Host、Typert、Remote 的类型入口和 `./types` 的 JS 入口；补齐引导安装的协议返回类型。
- 声明 Node.js 运行范围，并补充分发、DSH peer 依赖及所需权限说明；保留正常运行所需的 `zod`，不伪造外部市场安全认证。
- 提高市场展示密度：内容区 490px 起支持双列，取消卡片固定最小高度，压缩卡片和筛选区留白；
  新增 12 个插件的密度回归，验证 794px 高窗口首屏至少完整展示 4 个插件。
...
- 增加真实 npm tarball 的导出与声明依赖检查、公开入口导入、31 个 Typert 类型的 TypeScript 消费端检查，以及缺失文件/清单回归；CI 覆盖 Node 22.19.0 和 24。
